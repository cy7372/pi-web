...
test("claims only blocking extension requests and deduplicates their ids", async () => {
  ...
  // Passive custom overlays (toast notifications) must not claim attention.
  assert.equal(claimExtensionAttentionNotification({
    type: "extension_ui_request",
    id: "custom-passive",
    method: "custom",
    lines: ["[toast] build ok"],
  }, notifiedIds), false);
  assert.equal(claimExtensionAttentionNotification({
    type: "extension_ui_request",
    id: "custom-awaiting",
  ...
  assert.equal(claimExtensionAttentionNotification({
    type: "extension_ui_request",
    id: "custom-awaiting-closed",
    method: "custom",
    lines: [],
    awaiting: true,
    closed: true,
  }, notifiedIds), false);
  assert.deepEqual([...notifiedIds], ["confirm-1", "select-1", "input-1", "editor-1", "custom-awaiting"]);
});
