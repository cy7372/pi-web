...
test("get_tools preserves the SDK tool definition fields", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
...
test("built-in subagents persist their selected resource policy", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const subagentSource = await readFile(new URL("./subagent-runtime.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
...
test("running snapshots expose sessions with suppressed completion notifications", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const runningRouteSource = await readFile(new URL("../app/api/agent/running/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const sessionsRouteSource = await readFile(new URL("../app/api/sessions/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
...
test("RPC session startup opens an existing session file only once and trusts its cwd", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const routeSource = await readFile(new URL("../app/api/agent/[id]/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const eventRouteSource = await readFile(new URL("../app/api/agent/[id]/events/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const autoNameRouteSource = await readFile(new URL("../app/api/sessions/[id]/auto-name/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const deleteRouteSource = await readFile(new URL("../app/api/sessions/[id]/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const trustRouteSource = await readFile(new URL("../app/api/project-trust/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
...
test("new-session route applies model scope during construction instead of follow-up commands", async () => {
  const source = await readFile(new URL("../app/api/agent/new/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
...
test("prompt routes mark only preflight failures as rejected", async () => {
  const existingRoute = await readFile(new URL("../app/api/agent/[id]/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
  const newRoute = await readFile(new URL("../app/api/agent/new/route.ts", import.meta.url), "utf8").then((t) => t.replace(/\r\n/g, "\n"));
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(startupSource, /: readSessionToolSelection\(\s*[\s\S]*?sessionManager\.getEntries\(\) as unknown as SessionEntry\[\]/);
...
function createParkedPromptFakeInner() {
  const submitted = [];
  const state = { compacting: true, listener: null };
  const inner = {
    sessionId: "park-test",
    sessionFile: "",
    ...
    subscribe(cb) { state.listener = cb; return () => { state.listener = null; }; },
    prompt: (message, options) => {
      submitted.push({ message, streamingBehavior: options?.streamingBehavior });
      options.preflightResult?.(true);
      return Promise.resolve();
    },
    dispose() {},
  };
  return { inner, submitted, state };
}

test("prompts park during compaction and flush in order on compaction_end", async () => {
  const { inner, submitted, state } = createParkedPromptFakeInner();
  ...
  try {
    // While compacting, prompts park instead of surfacing the SDK error.
    assert.deepEqual(await wrapper.send({ type: "prompt", message: "first" }), { parked: true });
    assert.deepEqual(await wrapper.send({ type: "prompt", message: "second" }), { parked: true });
    assert.deepEqual(submitted, []);
    assert.equal(wrapper.isRunning(), true);

    // Parking surfaces a merged queue_update so the UI shows queued rows.
    const updates = events.filter((event) => event.type === "queue_update");
    assert.deepEqual(updates.at(-1).followUp, ["first", "second"]);

    // steer/follow_up are not parked — the SDK queues them natively.
    state.compacting = false;
    state.listener({ type: "compaction_end" });
    await new Promise((resolve) => setImmediate(resolve));
    assert.deepEqual(submitted, [
      { message: "first", streamingBehavior: undefined },
      { message: "second", streamingBehavior: undefined },
    ]);
  } finally {
    unsubscribe();
    wrapper.destroy();
...
test("parked prompt flushed into a live run joins the queue as followUp", async () => {
  const { inner, submitted, state } = createParkedPromptFakeInner();
  const wrapper = new AgentSessionWrapper(inner);
  wrapper.start();

  try {
    await wrapper.send({ type: "prompt", message: "held" });
    state.compacting = false;
    inner.isStreaming = true;
    state.listener({ type: "compaction_end" });
    await new Promise((resolve) => setImmediate(resolve));
    assert.deepEqual(submitted, [{ message: "held", streamingBehavior: "followUp" }]);
  } finally {
    wrapper.destroy();
  }
});

test("clear_queue recalls parked prompt texts and unblocks the wrapper", async () => {
  const { inner, state } = createParkedPromptFakeInner();
  const wrapper = new AgentSessionWrapper(inner);
  wrapper.start();
  ...
  try {
    ...
    assert.deepEqual(cleared, { steering: [], followUp: ["held"] });

    // Compaction ends — nothing flushes (the queue was recalled) and the
    // parked queue no longer keeps the wrapper running.
    state.compacting = false;
    state.listener({ type: "compaction_end" });
    await new Promise((resolve) => setImmediate(resolve));
...
test("awaiting custom UI stays pending, replays to new SSE subscribers, and closes cleanly", async () => {
  const { inner } = createParkedPromptFakeInner();
  inner.sessionId = "awaiting-custom-test";
  const wrapper = new AgentSessionWrapper(inner);
  wrapper.start();
  const live = [];
  const unsubscribeLive = wrapper.onEvent((event) => live.push(event));

  try {
    const ui = wrapper.createExtensionUiContext();
    let doneFn;
    const settled = ui.custom(
      (tui, theme, keybindings, done) => {
        doneFn = done;
        return { render: () => ["Continue?"], dispose() {} };
      },
      { overlayOptions: { awaiting: true } },
    );
    await new Promise((resolve) => setImmediate(resolve));

    // Interactive custom dialogs (overlayOptions.awaiting, ask_user_question)
    // count as awaiting input for the sidebar badge…
    assert.equal(wrapper.hasPendingUiRequests(), true);
    // …and live subscribers saw the render event carrying the awaiting flag.
    const liveEvent = live.find((e) => e.type === "extension_ui_request" && e.method === "custom" && !e.closed);
    assert.ok(liveEvent, "custom render event emitted");
    assert.equal(liveEvent.awaiting, true);
    assert.deepEqual(liveEvent.lines, ["Continue?"]);

    // Page re-entry: a NEW subscriber (fresh SSE stream) must receive the
    // pending request replayed immediately — this is the whole recovery path.
    const replayed = [];
    const unsubscribeReplay = wrapper.onEvent((event) => replayed.push(event));
    const replayEvent = replayed.find((e) => e.type === "extension_ui_request" && e.method === "custom" && !e.closed);
    assert.ok(replayEvent, "pending custom event replayed to new subscriber");
    assert.equal(replayEvent.awaiting, true);
    unsubscribeReplay();

    // Answering closes the panel: a closed:true event fires, the pending
    // map clears, and the custom() promise resolves with the answer.
    const closedPromise = new Promise((resolve) => {
      const stop = wrapper.onEvent((e) => {
        if (e.type === "extension_ui_request" && e.method === "custom" && e.closed) { stop(); resolve(e); }
      });
    });
    doneFn("yes");
    await closedPromise;
    assert.equal(wrapper.hasPendingUiRequests(), false);
    assert.equal(await settled, "yes");
  } finally {
    unsubscribeLive();
    wrapper.destroy();
  }
...
test("passive custom overlays do not flip the awaiting-input indicator", async () => {
  // 80cb661's original goal: compact-cache footers and toasts stay pinned in
  // pendingUiRequests but must NOT light the amber awaiting badge. Only
  // overlays that opt in via overlayOptions.awaiting do.
  const { inner } = createParkedPromptFakeInner();
  inner.sessionId = "passive-custom-test";
  const wrapper = new AgentSessionWrapper(inner);
  wrapper.start();
  ...
  try {
    const ui = wrapper.createExtensionUiContext();
    const settled = ui.custom(
      () => ({ render: () => ["cache: 42%"], dispose() {} }),
      {},
    );
    await new Promise((resolve) => setImmediate(resolve));
    assert.equal(wrapper.hasPendingUiRequests(), false);
    wrapper.destroy();
    // destroy() aborts the extension stop signal, which settles the pending
    // custom() promise with its default — assert it does not hang forever.
    ...
    try { wrapper.destroy(); } catch { /* already destroyed */ }
  }
});
