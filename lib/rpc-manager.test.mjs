...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(
    startupSource,
    /: readSessionToolSelection\(\s*[\s\S]*?sessionManager\.getEntries\(\) as unknown as SessionEntry\[\]/,
  );
  assert.match(
    startupSource,
...
test("crossing the Chat-only boundary persists and rebuilds the wrapper", async () => {
  ...
  assert.match(
    switchSource,
    /!hasCurrentResourcePolicy\s*\|\| existing\.isChatOnly\(\) !== \(toolNames\.length === 0\)/,
  );
  assert.match(
    switchSource,
...
test("prompts park during compaction and flush in order on compaction_end", async () => {
  ...
  try {
    ...
    assert.deepEqual(await wrapper.send({ type: "prompt", message: "first" }), {
      parked: true,
    });
    assert.deepEqual(
...
test("parked prompt flushed into a live run joins the queue as followUp", async () => {
  ...
  try {
    ...
    await new Promise((resolve) => setImmediate(resolve));
    assert.deepEqual(submitted, [
      { message: "held", streamingBehavior: "followUp" },
    ]);
  } finally {
    wrapper.destroy();
  }
...
test("awaiting custom UI stays pending, replays to new SSE subscribers, and closes cleanly", async () => {
  ...
  try {
    ...
    const liveEvent = live.find(
      (e) =>
        e.type === "extension_ui_request" && e.method === "custom" && !e.closed,
    );
    ...
    const replayEvent = replayed.find(
      (e) =>
        e.type === "extension_ui_request" && e.method === "custom" && !e.closed,
    );
