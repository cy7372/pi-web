...
const globalStyles = await readFile(new URL("../styles/app.css", import.meta.url), "utf8");
...
test("hides subagent rows and aggregates their state into the main session row", () => {
  ...
  assert.match(source, /familySessions\.some\(\s*\(session\) => session\.id === selectedSessionId\s*,?\s*\)/);
  assert.match(source, /familySessions\.some\(\(session\) =>\s*runningSessionIds\.has\(session\.id\)\s*,?\s*\)/);
