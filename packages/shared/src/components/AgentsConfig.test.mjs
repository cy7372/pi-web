...
test("offers a persisted built-in sub-agent switch with explicit session reload", () => {
  ...
  assert.match(
    source,
    /className="agents-concurrency-control"[\s\S]*?t\("agents\.maxConcurrent"\)/,
  );
  assert.equal(
    (source.match(/className="agents-feature-setting"/g) ?? []).length,
    1,
  );
  assert.match(
    cssSource,
  ...
  assert.match(
    cssSource,
    /\.agents-concurrency-control \{[\s\S]*?white-space: nowrap;/,
  );
});
