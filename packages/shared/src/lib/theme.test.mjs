...
test("public/theme-init.js mirrors THEME_INIT_SCRIPT (layout loads it as the render-blocking theme initializer)", () => {
  ...
  for (const source of [file, THEME_INIT_SCRIPT]) {
    assert.ok(source.includes('localStorage.getItem("pi-theme")'), "reads the stored theme preference");
    assert.ok(source.includes(themeIds), "validates the stored id against THEME_OPTIONS");
    assert.ok(source.includes("prefers-color-scheme: dark"), "falls back to the system preference");
    assert.ok(source.includes("dataset.theme"), "sets data-theme before first paint");
    assert.ok(source.includes('classList.toggle("dark"'), "toggles the dark class");
  }
  assert.ok(file.startsWith("// AUTO-GENERATED from lib/theme.ts"), "generated banner intact");
});
