...
function agentHeavyChain(turns) {
  ...
  for (let t = 0; t < turns; t++) {
    ...
    push("message", {
      role: "assistant",
      content: [
        { type: "tool_call", id: `tc${t}`, name: "bash", arguments: {} },
      ],
    });
    push("message", {
      role: "toolResult",
      content: [{ type: "tool_result", toolUseId: `tc${t}`, output: "ok" }],
    });
...
test("buildSessionContext hasMore stays true when the visible window truncates", () => {
  ...
  const ctx = buildSessionContext(entries, entries[entries.length - 1].id, {
    tail: 10,
  });
  assert.equal(ctx.hasMore, true);
