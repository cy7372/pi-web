...
Dancher Agent web treats an explicitly empty tool selection as **Chat only**, not merely as
...
project `AGENTS.md`, `AGENTS.override.md`, and `CLAUDE.md` files. Dancher Agent web does not
...
tools are activated alongside the profile's built-in tools except for Dancher Agent web's
