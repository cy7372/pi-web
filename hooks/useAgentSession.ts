...
function probeUserKey(m: unknown): string | null {
  if (!m || typeof m !== "object" || (m as { role?: unknown }).role !== "user")
    return null;
  try {
    return userMessageKey(m as Parameters<typeof userMessageKey>[0]).slice(
      0,
      120,
    );
  } catch {
...
function probeScan(source: string, prev: AgentMessage[], next: AgentMessage[]) {
  ...
  for (const m of next) {
    ...
    if (seen.has(k)) {
      console.warn(
        `[dup-probe] DUPLICATE user message after ${source}: key=${k} prevLen=${prev.length} nextLen=${next.length}`,
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const noticeTexts = useMemo<AgentNoticeTexts>(
    () => ({
      extensionTurnStart:
        opts.noticeTexts?.extensionTurnStart ?? "Extension triggered a new run",
  ...
  const setMessages = useCallback(
    (
      ...
      setMessagesBase((prev) => {
        ...
        if (next !== prev)
          probeScan(
            probeTag || probeTagRef.current || "setMessages",
            prev,
            next,
          );
        return next;
  ...
  const loadSession = useCallback(
    ...
    [setMessages, setToolPresetState],
  );
      ...
      switch (event.type) {
          ...
          if (event.type === "message_start") {
            ...
            if (msg?.role === "assistant") {
              ...
              if (msg.role === "custom") {
                ...
                addNotice({
                  type: "info",
                  message: noticeTexts.extensionTurnStart,
                });
              }
              setAgentPhase(null);
            }
        ...
        case "message_end": {
          ...
          if (completed && completed.role === "user") {
            ...
            const stopReason = (completed as { stopReason?: string })
              .stopReason;
            ...
            const blocks = (("content" in completed ? completed.content : []) ??
            ...
            if (!aborted || hasContent) {
              setMessages(
                (prev) => [...prev, normalizeToolCalls(completed)],
                "message_end:other",
              );
            }
            if (aborted) {
    ...
    if (session) {
      ...
      loadSession(session.id, true, true).then((agentState) => {
        ...
        if (agentState?.state) {
          ...
          if (agentState.state.autoCompactionEnabled !== undefined)
            setAutoCompactionEnabled(agentState.state.autoCompactionEnabled);
