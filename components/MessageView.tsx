...
function CompactionMessageView({
  ...
  if (parsedSummary.readFiles.length > 0)
    countParts.push(
      t("i18n.compactFilesReadCount", {
        count: parsedSummary.readFiles.length,
      }),
    );
  if (parsedSummary.modifiedFiles.length > 0)
    countParts.push(
      t("i18n.compactFilesModifiedCount", {
        count: parsedSummary.modifiedFiles.length,
      }),
    );
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      >
        <button
          ...
          aria-expanded={expanded}
          title={
            expanded ? t("i18n.compactionCollapse") : t("i18n.compactionExpand")
          }
          aria-label={
            expanded ? t("i18n.compactionCollapse") : t("i18n.compactionExpand")
          }
...
function CompactionFileMetadata({
  ...
  if (readFiles.length > 0)
    parts.push(t("i18n.compactFilesReadCount", { count: readFiles.length }));
  if (modifiedFiles.length > 0)
    parts.push(
      t("i18n.compactFilesModifiedCount", { count: modifiedFiles.length }),
    );
        ...
        {files.map((file) =>
          onOpenFile ? (
            <li key={file}>
              <button
                type="button"
                className="compaction-file-link"
                title={file}
