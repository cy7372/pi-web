...
function CompactionMessageView({ message, cwd, onOpenFile }: { message: CustomMessage; cwd?: string; onOpenFile?: (filePath: string) => void }) {
  ...
  if (parsedSummary.readFiles.length > 0)
    countParts.push(t("i18n.compactFilesReadCount", { count: parsedSummary.readFiles.length }));
  if (parsedSummary.modifiedFiles.length > 0)
    countParts.push(t("i18n.compactFilesModifiedCount", { count: parsedSummary.modifiedFiles.length }));
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      >
        ...
        >
          ...
          <svg width={10} height={10} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ transform: expanded ? "rotate(180deg)" : "none", transition: "transform 0.15s", flexShrink: 0 }}>
            <polyline points="6 9 12 15 18 9" />
          </svg>
        </button>
